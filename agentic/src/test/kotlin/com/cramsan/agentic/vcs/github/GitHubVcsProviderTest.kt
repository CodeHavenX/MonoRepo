...
class GitHubVcsProviderTest : CoroutineTest() {
    ...
    @Test
    fun `createPullRequest assembles correct gh command and maps response`() =
        runCoroutineTest {
            val cannedResponse =
                """
                {
            ...
            coVerify {
                shell.run(
                    match<List<String>> { args ->
                        args.containsAll(
                            listOf("gh", "pr", "create", "--title", "My PR", "--body", "Body", "--base", "main", "--head", "feature/x", "--repo", "owner/repo"),
                        )
                    },
                )
            }
    ...
    @Test
    fun `getPullRequestComments maps JSON comment array correctly`() =
        runCoroutineTest {
            val cannedResponse =
                """
                {
    ...
    @Test
    fun `pullRequestHasRequestedChanges returns true when CHANGES_REQUESTED`() =
        runCoroutineTest {
    ...
    @Test
    fun `pullRequestHasRequestedChanges returns false when APPROVED`() =
        runCoroutineTest {
            coEvery { shell.run(*anyVararg()) } returns
                ShellResult(
    ...
    @Test
    fun `createPullRequest throws VcsProviderException on non-zero exit code`() =
        runCoroutineTest {
