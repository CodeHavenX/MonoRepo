...
class FlippableController {
    ...
    private val flipRequestsMutable = MutableSharedFlow<FlippableState>(extraBufferCapacity = 1)
    internal val flipRequests = flipRequestsMutable.asSharedFlow()

    private var currentSide: FlippableState = FlippableState.FRONT
    private var flipEnabled: Boolean = true
    ...
    fun flip(flippableState: FlippableState) {
        ...
        flipRequestsMutable.tryEmit(flippableState)
    }
     ...
     */
    fun flip() {
        if (currentSide == FlippableState.FRONT) {
            flipToBack()
        } else {
            flipToFront()
        }
    ...
    internal fun setConfig(flipEnabled: Boolean = true) {
 ...
 */
@Composable
fun rememberFlipController(): FlippableController = remember {
    FlippableController()
}
