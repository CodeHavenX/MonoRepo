...
class DefaultOrchestratorTest : CoroutineTest() {
    ...
    @Test
    fun `all tasks DONE on first tick triggers RunCompleted`() =
        runCoroutineTest {
    ...
    @Test
    fun `no progress triggers RunDeadlocked`() =
        runCoroutineTest {
            val task = makeTask("task-1")
            coEvery { taskListProvider.provide() } returns listOf(task)
    ...
    @Test
    fun `PENDING task causes agent launch and getOrCreate is called`() =
        runCoroutineTest {
            val task = makeTask("task-1")
            coEvery { taskListProvider.provide() } returns listOf(task)
    ...
    @Test
    fun `pool sizing limits concurrent agents to agentPoolSize`() =
        runCoroutineTest {
    ...
    @Test
    fun `priority ordering - task with higher downstreamCount is assigned first`() =
        runCoroutineTest {
    ...
    @Test
    fun `AgentResult_Failed causes TaskFailed notification`() =
        runCoroutineTest {
            val task = makeTask("task-1")
            coEvery { taskListProvider.provide() } returns listOf(task)
