...
class VcsCommentNotifierTest {
    ...
    @Test
    fun `TaskFailed event posts comment to matching open PR`() =
        runTest {
    ...
    @Test
    fun `TaskFailed event with no open PR does not call addPullRequestComment`() =
        runTest {
    ...
    @Test
    fun `duplicate notification prevention - does not post if marker already exists`() =
        runTest {
            val existingComment =
                PullRequestComment(
                    author = "agentic-bot",
    ...
    @Test
    fun `RunCompleted event makes no PR API calls`() =
        runTest {
