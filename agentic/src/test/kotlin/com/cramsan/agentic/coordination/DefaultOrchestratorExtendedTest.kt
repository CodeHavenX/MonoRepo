...
class DefaultOrchestratorExtendedTest : CoroutineTest() {
    ...
    @Test
    fun `worktree is deleted when task transitions to DONE`() =
        runCoroutineTest {
            val task = makeTask("task-1")
            coEvery { taskListProvider.provide() } returns listOf(task)
            var tick = 0
            coEvery {
                stateDeriver.statusOf(
                    task,
                    any(),
                    any(),
                )
            } answers { if (tick++ == 0) TaskStatus.PENDING else TaskStatus.DONE }
    ...
    @Test
    fun `worktree is deleted when task transitions to FAILED`() =
        runCoroutineTest {
            val task = makeTask("task-1")
            coEvery { taskListProvider.provide() } returns listOf(task)
            var tick = 0
            ...
            } answers { if (tick++ == 0) TaskStatus.PENDING else TaskStatus.FAILED }
            coEvery { dependencyGraph.downstreamCount(any()) } returns 0
            coEvery { dependencyGraph.dependentsOf(any()) } returns emptySet()
            coEvery { worktreeManager.getOrCreate("task-1") } returns worktree("task-1")
    ...
    @Test
    fun `worktree cleanup does not happen twice for the same task`() =
        runCoroutineTest {
            val task = makeTask("task-1")
            coEvery { taskListProvider.provide() } returns listOf(task)
            var tick = 0
            coEvery { stateDeriver.statusOf(task, any(), any()) } answers {
                when (tick++) {
                    0 -> TaskStatus.PENDING else -> TaskStatus.DONE
                }
            }
            coEvery { dependencyGraph.downstreamCount(any()) } returns 0
            coEvery { dependencyGraph.dependentsOf(any()) } returns emptySet()
    ...
    @Test
    fun `IN_REVIEW task does not launch agent — agent slot stays free for PENDING tasks`() =
        runCoroutineTest {
    ...
    @Test
    fun `all tasks BLOCKED or FAILED but deadlock not declared because active agents still running`() =
        runCoroutineTest {
            val task = makeTask("task-1")
            coEvery { taskListProvider.provide() } returns listOf(task)
    ...
    @Test
    fun `RunCompleted event includes all completed tasks`() =
        runCoroutineTest {
    ...
    @Test
    fun `RunDeadlocked event contains blocked and failed tasks`() =
        runCoroutineTest {
    ...
    @Test
    fun `only agentPoolSize agents launched per tick`() =
        runCoroutineTest {
    ...
    @Test
    fun `agent is not re-launched for task already in activeTaskIds`() =
        runCoroutineTest {
            val task = makeTask("task-1")
            coEvery { taskListProvider.provide() } returns listOf(task)
            var tick = 0
    ...
    @Test
    fun `dependent task is not assigned before its dependency is done`() =
        runCoroutineTest {
