package com.cramsan.edifikana.client.lib.features.admin.timecardemployeelist
...
import com.cramsan.edifikana.client.lib.features.management.employeelist.EmployeeMemberUIModel
...
class EmployeeListViewModelTest : CoroutineTest() {
    ...
    fun `test navigateToAddPrimaryEmployee emits NavigateToScreen event`() = runCoroutineTest {
        val verificationJob = launch {
            windowEventBus.events.test {
                assertEquals(
                    EdifikanaWindowsEvent.NavigateToScreen(ManagementDestination.AddPrimaryEmployeeManagementDestination),
                    awaitItem()
                )
            }
        }
    ...
    fun `test navigateToAddSecondaryEmployee emits NavigateToScreen event`() = runCoroutineTest {
        val verificationJob = launch {
            windowEventBus.events.test {
                assertEquals(
                    EdifikanaWindowsEvent.NavigateToScreen(ManagementDestination.AddSecondaryEmployeeManagementDestination),
                    awaitItem()
                )
            }
        }
    ...
    fun `test navigateToEmployee emits NavigateToScreen event`() = runCoroutineTest {
        ...
        val verificationJob = launch {
            windowEventBus.events.test {
                assertEquals(
                    EdifikanaWindowsEvent.NavigateToScreen(ManagementDestination.EmployeeDestination(employeeId)),
                    awaitItem()
                )
            }
        }
