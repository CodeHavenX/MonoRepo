...
class ClaudeReviewerAgentNegativeTest {
    ...
    @Test
    fun `reviewDocuments propagates AiProviderException from AI provider`() =
        runTest {
    ...
    @Test
    fun `reviewDocuments propagates generic RuntimeException from AI provider`() =
        runTest {
    ...
    @Test
    fun `reviewDocuments returns fallback text when AI response has no content blocks`() =
        runTest {
    ...
    @Test
    fun `reviewDocuments returns fallback text when AI response contains only ToolCall blocks`() =
        runTest {
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
                    id = "r1",
    ...
    @Test
    fun `reviewDocuments with empty document list still calls AI provider`() =
        runTest {
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
                    id = "r1",
    ...
    @Test
    fun `reviewDocuments with empty reviewer name still returns feedback with that empty name`() =
        runTest {
    ...
    @Test
    fun `reviewCode propagates AiProviderException from AI provider`() =
        runTest {
            coEvery {
                aiProvider.chat(
                    any(),
            ...
            } throws AiProviderException("Quota exceeded", exitCode = 429)
    ...
    @Test
    fun `reviewCode propagates generic RuntimeException from AI provider`() =
        runTest {
    ...
    @Test
    fun `reviewCode returns fallback text when AI response has no content blocks`() =
        runTest {
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
                    id = "r1",
    ...
    @Test
    fun `reviewCode returns fallback text when AI response contains only ToolCall blocks`() =
        runTest {
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
                    id = "r1",
    ...
    @Test
    fun `reviewCode with empty diff still calls AI provider and returns feedback`() =
        runTest {
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
                    id = "r1",
