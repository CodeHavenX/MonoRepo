...
class DefaultAgentRunnerExtendedTest {
    ...
    private fun makeRunner(vararg agents: ReviewerAgent) =
        DefaultAgentRunner(
            agentSession,
            vcsProvider,
            agents.toList(),
    ...
    @Test
    fun `reviewer comment contains stable HTML marker with reviewer name`() =
        runTest {
            ...
            coEvery {
                reviewerAgent1.reviewCode(
                    reviewerDef,
                    task,
                    any(),
                )
            } returns ReviewerFeedback("security", "All clear")
    ...
    @Test
    fun `reviewer comment includes reviewer name in heading`() =
        runTest {
            ...
            } returns ReviewerFeedback("performance", "Fast enough")
    ...
    @Test
    fun `reviewer comment includes the feedback content`() =
        runTest {
            ...
            } returns ReviewerFeedback("security", feedbackContent)
    ...
    @Test
    fun `multiple reviewers each post their own comment on the PR`() =
        runTest {
            ...
            } returns ReviewerFeedback("performance", "Perf OK")
    ...
    @Test
    fun `two reviewer agents produce two comments for each reviewer definition`() =
        runTest {
            ...
            } returns ReviewerFeedback("design", "Agent1 view")
            coEvery {
                reviewerAgent2.reviewCode(
                    reviewerDef,
                    task,
                    any(),
                )
            } returns ReviewerFeedback("design", "Agent2 view")
    ...
    @Test
    fun `reviewer agent failure does not fail the overall run`() =
        runTest {
    ...
    @Test
    fun `failed_txt contains the exact failure reason from agent`() =
        runTest {
    ...
    @Test
    fun `failed_txt timeout message mentions the configured timeout seconds`() =
        runTest {
    ...
    @Test
    fun `reviewers are not invoked when agent fails`() =
        runTest {
