...
class DefaultAgentSessionTest {
    ...
    fun setup() {
        ...
        session = DefaultAgentSession(claudeClient, vcsProvider, shell, "claude-opus-4-6", "main", documentStore)

        // Default: no open PRs, no git diff
    ...
    fun `task_complete tool response returns AgentResult_PrOpened`() = runTest {
        ...
        coEvery { claudeClient.chat(any(), any(), any(), any()) } returns ClaudeResponse(
            id = "resp-1",
            content = listOf(ClaudeContentBlock.ToolUse("tool-1", "task_complete", taskCompleteInput)),
            stopReason = "tool_use",
        )
    ...
    fun `task_failed tool response returns AgentResult_Failed`() = runTest {
        ...
        coEvery { claudeClient.chat(any(), any(), any(), any()) } returns ClaudeResponse(
            id = "resp-1",
            content = listOf(ClaudeContentBlock.ToolUse("tool-1", "task_failed", taskFailedInput)),
            stopReason = "tool_use",
        )
    ...
    fun `read_file tool reads existing file and appends content to messages`() = runTest {
        ...
        var callCount = 0
        coEvery { claudeClient.chat(any(), any(), any(), any()) } answers {
            callCount++
            if (callCount == 1) {
                ClaudeResponse(
                    id = "resp-1",
                    content = listOf(ClaudeContentBlock.ToolUse("tool-1", "read_file", readFileInput)),
                    stopReason = "tool_use",
                )
            } else {
                ClaudeResponse(
                    id = "resp-2",
                    content = listOf(ClaudeContentBlock.ToolUse("tool-2", "task_complete", taskCompleteInput)),
                    stopReason = "tool_use",
                )
            }
        }
        ...
        assertIs<AgentResult.PrOpened>(result)
        // The second chat call should include the file content in messages
        // We verify this by checking the call count was 2
        assertTrue(callCount == 2)
    }

    @Test
    fun `run_command tool result is returned to agent`() = runTest {
        coEvery { shell.run("sh", "-c", "echo hello") } returns ShellResult("hello\n", 0, "")
        ...
        coEvery { claudeClient.chat(any(), any(), any(), any()) } answers {
            ...
            if (callCount == 1) {
                ClaudeResponse(
                    id = "resp-1",
                    content = listOf(ClaudeContentBlock.ToolUse("tool-1", "run_command", runCommandInput)),
                    stopReason = "tool_use",
                )
            } else {
                ClaudeResponse(
