...
class AddEmployeeViewModel @Inject constructor(
    ...
    fun saveEmployee(
        ...
        if (
            ...
            _event.emit(AddEmployeeEvent.TriggerMainActivityEvent(
        ...
        if (result.isFailure || result.isFailure) {
            _event.emit(AddEmployeeEvent.TriggerMainActivityEvent(
                // TODO: Add compose resource loading
                MainActivityEvent.ShowSnackbar(applicationContext.getString(R.string.text_there_was_an_error_processing_request))
            ))
        } else {
            _event.emit(AddEmployeeEvent.TriggerMainActivityEvent(
