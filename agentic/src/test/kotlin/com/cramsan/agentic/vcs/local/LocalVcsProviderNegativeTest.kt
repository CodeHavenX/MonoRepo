...
class LocalVcsProviderNegativeTest : CoroutineTest() {
    ...
    @Test
    fun `isPullRequestMerged throws for unknown prId`() = runCoroutineTest {
    ...
    @Test
    fun `getPullRequestComments throws for unknown prId`() = runCoroutineTest {
        val provider = manualMergeProvider(testCoroutineDispatcher)
    ...
    @Test
    fun `addPullRequestComment throws for unknown prId`() = runCoroutineTest {
        val provider = manualMergeProvider(testCoroutineDispatcher)
    ...
    @Test
    fun `pullRequestHasRequestedChanges throws for unknown prId`() = runCoroutineTest {
        val provider = manualMergeProvider(testCoroutineDispatcher)
    ...
    @Test
    fun `autoMerge=true git merge failure throws VcsProviderException`() = runCoroutineTest {
    ...
    @Test
    fun `createPullRequest throws when an open PR already exists for the same branch`() = runCoroutineTest {
        val provider = manualMergeProvider(testCoroutineDispatcher)
        provider.createPullRequest("feature/a", "main", "PR 1", "Body")
    ...
    @Test
    fun `isPullRequestMerged with autoMerge=false throws VcsProviderException on git failure`() = runCoroutineTest {
        val provider = manualMergeProvider(testCoroutineDispatcher)
        provider.createPullRequest("feature/a", "main", "PR 1", "Body")
        coEvery { shell.run(*anyVararg(), workingDir = any()) } returns ShellResult(
            "",
    ...
    @Test
    fun `creating a second PR on the same branch is allowed after the first is no longer OPEN`() = runCoroutineTest {
        val provider = autoMergeProvider(testCoroutineDispatcher)
