...
class FakeAiProviderTest {
    ...
    @Test
    fun `TEST mode returns queued response`() =
        runTest {
    ...
    @Test
    fun `TEST mode throws when queue is empty`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.TEST)
    ...
    @Test
    fun `TEST mode returns responses in FIFO order`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.TEST)
    ...
    @Test
    fun `TEST mode captures requests`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.TEST)
    ...
    @Test
    fun `DEMO mode generates response when queue empty`() =
        runTest {
    ...
    @Test
    fun `DEMO mode prefers queued response over generation`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.DEMO)
    ...
    @Test
    fun `DEMO mode auto-completes after configured turns`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.DEMO, autoCompleteAfterTurns = 2)
            val taskCompleteTool =
                AiTool(
                    "task_complete",
                    "Complete the task",
    ...
    @Test
    fun `DEMO mode does not auto-complete if task_complete tool not available`() =
        runTest {
    ...
    @Test
    fun `DEMO mode uses default text response`() =
        runTest {
    ...
    @Test
    fun `DEMO mode selects list_files tool for begin message`() =
        runTest {
    ...
    @Test
    fun `DEMO mode selects read_file tool for read message`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.DEMO, autoCompleteAfterTurns = 100)
    ...
    @Test
    fun `enqueueToolCallResponse creates correct structure`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.TEST)
    ...
    @Test
    fun `enqueueTextResponse creates correct structure`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.TEST)
    ...
    @Test
    fun `clearQueue only clears queue not captured requests`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.DEMO)
    ...
    @Test
    fun `clearCapturedRequests only clears requests not queue`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.DEMO)
            provider.enqueueTextResponse("queued")
            provider.chat("system", emptyList(), emptyList())
    ...
    @Test
    fun `delay configuration is accepted`() =
        runTest {
    ...
    @Test
    fun `response IDs are unique and prefixed with fake-`() =
        runTest {
            val provider = FakeAiProvider(mode = FakeMode.DEMO)
