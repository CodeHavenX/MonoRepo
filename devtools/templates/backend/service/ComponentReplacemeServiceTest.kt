 ...
 *
 * Tests business logic in isolation. Uses [ExampleComponentReplacemeDatastore] as a
 * lightweight in-memory datastore. For tests that require specific datastore behavior,
 * replace it with a MockK mock:
 * ```
 * private val datastore: ComponentReplacemeDatastore = mockk()
 *
 * @BeforeTest
 * fun setUp() {
 *     coEvery { datastore.create(any()) } returns Result.success(ComponentReplaceme(...))
 *     service = ComponentReplacemeService(datastore)
 * }
 * ```
 *
