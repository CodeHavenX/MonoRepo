...
class LocalVcsProvider(
    ...
    override suspend fun createPullRequest(
        ...
        withContext(dispatcher) {
            logI(
                TAG,
                "createPullRequest: sourceBranch=$sourceBranch, targetBranch=$targetBranch, title='$title', labels=$labels, autoMerge=$autoMerge",
            )
            mutex.withLock {
                val state = readState()
                val existing =
                ...
                if (autoMerge) {
                    ...
                    if (result.exitCode != 0) {
                        logE(
                            TAG,
                            "createPullRequest: git merge failed: exitCode=${result.exitCode}, stderr=${result.stderr}",
                        )
    ...
    override suspend fun getPullRequestComments(prId: String): List<PullRequestComment> =
        withContext(dispatcher) {
            ...
            val pr =
                mutex.withLock { readState() }.prs.find { it.id == prId }
        ...
        withContext(dispatcher) {
            ...
            mutex.withLock {
                ...
                if (result.exitCode != 0) {
                    logE(
                        TAG,
                        "isPullRequestMerged: git branch --merged failed: exitCode=${result.exitCode}, stderr=${result.stderr}",
                    )
                    throw VcsProviderException(
                ...
                if (isMerged) {
                    logI(
                        TAG,
                        "isPullRequestMerged: prId=$prId branch ${pr.sourceBranch} found in merged branches, updating state",
                    )
                    pr.state = PullRequestState.MERGED
                    writeState(state)
                }
...
private fun LocalPr.toPullRequest() =
    PullRequest(
        id = id,
        url = "local://pr/$id",
        title = title,
