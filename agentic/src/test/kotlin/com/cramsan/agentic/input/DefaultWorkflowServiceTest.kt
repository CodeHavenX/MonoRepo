...
class DefaultWorkflowServiceTest {
    ...
    @Test
    fun `startStage writes AI response to output file`() =
        runTest {
            every { documentStore.getAll() } returns emptyList()
    ...
    @Test
    fun `startStage includes input document content in AI prompt`() =
        runTest {
            ...
            coEvery { aiProvider.chat(any(), capture(capturedMessages), any()) } returns
                AiResponse(
                    id = "r1",
                    content = listOf(AiContentBlock.Text("# High-Level Plan")),
                    stopReason = "end_turn",
    ...
    @Test
    fun `startStage throws when AI returns no text content`() =
        runTest {
            every { documentStore.getAll() } returns emptyList()
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
    ...
    @Test
    fun `startStage throws for unknown stage ID`() =
        runTest {
    ...
    @Test
    fun `startStage throws when dependency is not approved`() =
        runTest {
    ...
    @Test
    fun `startStage includes dependency output in AI prompt`() =
        runTest {
            Files.writeString(docsDir.resolve("high-level-plan.md"), "# High-Level Plan\nApproved content")
            writeApprovalRecord("stage1")
            every { documentStore.getAll() } returns emptyList()
    ...
    @Test
    fun `startNextStage returns null when documents not validated`() =
        runTest {
            every { documentStore.getAll() } returns listOf(sampleDoc)
    ...
    @Test
    fun `startNextStage returns null when workflow is complete`() =
        runTest {
            every { documentStore.getAll() } returns listOf(sampleDoc)
    ...
    @Test
    fun `reviseStage throws when output file does not exist`() =
        runTest {
            assertFailsWith<IllegalArgumentException> {
    ...
    @Test
    fun `reviseStage overwrites file with AI-revised content`() =
        runTest {
    ...
    @Test
    fun `reviseStage sends annotated file content to AI`() =
        runTest {
