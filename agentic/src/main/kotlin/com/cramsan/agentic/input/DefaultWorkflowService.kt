 ...
 * `agentic plan`. Each stage is generated by the AI, reviewed and possibly revised by the
 * human operator, and then approved before the next stage begins.
 *
 * **State derivation**: [getState] determines the current workflow position by inspecting which
 * stage approval records (`.agentic-meta/stage.{id}.json`) exist on disk, and computes
 * approval drift warnings by hashing current input document content against the hashes recorded
 * at approval time.
 *
 * **AI generation**: [startStage] sends the stage's system prompt (from [WorkflowPromptConfig])
 * plus all input documents to [aiProvider] and writes the resulting text to the stage's
 * [com.cramsan.agentic.core.WorkflowStageConfig.outputFile].
 *
 * **Revision**: [reviseStage] re-sends the current stage output file — which the human may
 * have annotated inline — to the AI using [REVISION_SYSTEM_PROMPT] to produce a clean revised version.
 *
 * **Approval drift**: if any input document changes after a stage is approved, [getState] emits
 * a [com.cramsan.agentic.core.StageApprovalWarning]. This is advisory only — it does not block
 * subsequent stages from being generated or approved.
 */
