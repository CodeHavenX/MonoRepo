...
class DefaultStateDeriverExtendedTest {
    ...
    private fun makeMergedPr(sourceBranch: String) =
        PullRequest(
            id = "pr-1",
            url = "url",
            title = "PR",
    ...
    private fun makeOpenPr(sourceBranch: String) =
        PullRequest(
            id = "pr-1",
            url = "url",
            title = "PR",
    ...
    @Test
    fun `merged PR beats failed_txt — DONE takes priority`() =
        runTest {
    ...
    @Test
    fun `merged PR beats open PR — DONE takes priority`() =
        runTest {
    ...
    @Test
    fun `merged PR for different branch does not return DONE`() =
        runTest {
    ...
    @Test
    fun `failed_txt beats open PR — FAILED takes priority`() =
        runTest {
            val failedFile = agenticDir.resolve("tasks/task-001/failed.txt")
            Files.createDirectories(failedFile.parent)
    ...
    @Test
    fun `failed_txt beats worktree — FAILED takes priority`() =
        runTest {
            val failedFile = agenticDir.resolve("tasks/task-001/failed.txt")
            Files.createDirectories(failedFile.parent)
            ...
            } returns Worktree("task-001", Path.of("/tmp/wt"), "agentic/task-001")
    ...
    @Test
    fun `unblocked_txt overrides BLOCKED to PENDING when deps not done`() =
        runTest {
    ...
    @Test
    fun `unblocked_txt is deleted after use — one-shot behavior`() =
        runTest {
            val taskWithDeps = task.copy(dependencies = listOf("dep-task"))
            val unblockedFile = agenticDir.resolve("tasks/task-001/unblocked.txt")
            Files.createDirectories(unblockedFile.parent)
    ...
    @Test
    fun `second call after unblocked_txt deleted returns BLOCKED`() =
        runTest {
            val taskWithDeps = task.copy(dependencies = listOf("dep-task"))
            val unblockedFile = agenticDir.resolve("tasks/task-001/unblocked.txt")
            Files.createDirectories(unblockedFile.parent)
    ...
    @Test
    fun `unblocked_txt does not affect task that is already PENDING`() =
        runTest {
            val unblockedFile = agenticDir.resolve("tasks/task-001/unblocked.txt")
            Files.createDirectories(unblockedFile.parent)
    ...
    @Test
    fun `task with deps all DONE returns PENDING`() =
        runTest {
    ...
    @Test
    fun `task with dep IN_REVIEW returns BLOCKED — IN_REVIEW is not DONE`() =
        runTest {
            val taskWithDeps = task.copy(dependencies = listOf("dep-a"))
            assertEquals(
                TaskStatus.BLOCKED,
                deriver.statusOf(taskWithDeps, mapOf("dep-a" to TaskStatus.IN_REVIEW), makeContext()),
            )
        }

    @Test
    fun `task with dep IN_PROGRESS returns BLOCKED`() =
        runTest {
            val taskWithDeps = task.copy(dependencies = listOf("dep-a"))
            assertEquals(
                TaskStatus.BLOCKED,
                deriver.statusOf(taskWithDeps, mapOf("dep-a" to TaskStatus.IN_PROGRESS), makeContext()),
            )
        }

    @Test
    fun `task with dep FAILED returns BLOCKED`() =
        runTest {
            val taskWithDeps = task.copy(dependencies = listOf("dep-a"))
            assertEquals(
                TaskStatus.BLOCKED,
                deriver.statusOf(taskWithDeps, mapOf("dep-a" to TaskStatus.FAILED), makeContext()),
            )
        }

    @Test
    fun `task with dep BLOCKED returns BLOCKED`() =
        runTest {
            val taskWithDeps = task.copy(dependencies = listOf("dep-a"))
            assertEquals(
                TaskStatus.BLOCKED,
                deriver.statusOf(taskWithDeps, mapOf("dep-a" to TaskStatus.BLOCKED), makeContext()),
            )
        }

    @Test
    fun `task with mixed deps — one DONE one PENDING — returns BLOCKED`() =
        runTest {
            val taskWithDeps = task.copy(dependencies = listOf("dep-a", "dep-b"))
            assertEquals(
                TaskStatus.BLOCKED,
                deriver.statusOf(
                    taskWithDeps,
                    mapOf("dep-a" to TaskStatus.DONE, "dep-b" to TaskStatus.PENDING),
                    makeContext(),
                ),
            )
        }
    ...
    @Test
    fun `task with empty resolvedDependencies falls back to BLOCKED`() =
        runTest {
            val taskWithDeps = task.copy(dependencies = listOf("dep-a"))
    ...
    @Test
    fun `open PR for different task does not affect this task`() =
        runTest {
            coEvery { vcsProvider.pullRequestHasRequestedChanges(any()) } returns false
    ...
    @Test
    fun `worktree for different task does not affect this task`() =
        runTest {
            ...
            } returns Worktree("other-task", Path.of("/tmp/wt"), "agentic/other-task")
