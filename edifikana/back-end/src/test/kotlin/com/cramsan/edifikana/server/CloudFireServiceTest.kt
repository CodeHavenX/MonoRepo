...
class CloudFireServiceTest {
    ...
    fun `demo of serializing event log created`() {
        ...
        val eventData = DocumentEventData.newBuilder().apply {
            valueBuilder.apply {
                name = "projects/edifikana/databases/(default)/documents/eventLog/DNI_47524811-1715189619-MAINTENANCE_SERVICE"

                putFields("description", Value.newBuilder().apply {
                    stringValue = "ingresaron personal para servicio de jardinería Fernando Enrique Plano DNI 1658254, la srta. administradora les mostró detalladamente los lugares que se debe realizar los trabajos. El sr. jardinero indicó que hay plantas enfermas y que necesitan fumigar. a parte de la srta. administradora la sra. Claudia Bonani también está enseñando al jardinero los lugares a trabajar."
                }.build())

                putFields("employeeDocumentId", Value.newBuilder().apply {
                    stringValue = "DNI_47524811"
                }.build())

                putFields("eventType", Value.newBuilder().apply {
                    stringValue = "MAINTENANCE_SERVICE"
                }.build())

                putFields("fallbackEmployeeName", Value.newBuilder().apply {
                    stringValue = ""
                }.build())

                putFields("fallbackEventType", Value.newBuilder().apply {
                    stringValue = ""
                }.build())

                putFields("summary", Value.newBuilder().apply {
                ...
                putFields("timeRecorded", Value.newBuilder().apply {
                    integerValue = 1715189619
                }.build())

                putFields("unit", Value.newBuilder().apply {
                    stringValue = "109"
                }.build())
    ...
    fun `demo of serializing time card event created`() {
        ...
        val eventData = DocumentEventData.newBuilder().apply {
            valueBuilder.apply {
                name = "projects/edifikana/databases/(default)/documents/timeCardRecords/DNI_77135418-CLOCK_IN-1716076306"

                putFields("employeeDocumentId", Value.newBuilder().apply {
                    stringValue = "DNI_77135418"
                }.build())

                putFields("eventTime", Value.newBuilder().apply {
                ...
                putFields("imageUrl", Value.newBuilder().apply {
