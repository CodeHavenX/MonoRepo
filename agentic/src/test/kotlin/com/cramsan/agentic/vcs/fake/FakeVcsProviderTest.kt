...
class FakeVcsProviderTest {
    @Test
    fun `createPullRequest returns PR with state OPEN and increments IDs`() =
        runTest {
    ...
    @Test
    fun `mergePullRequest causes isPullRequestMerged to return true`() =
        runTest {
            val provider = FakeVcsProvider()
    ...
    @Test
    fun `requestChanges causes pullRequestHasRequestedChanges to return true`() =
        runTest {
            val provider = FakeVcsProvider()
            val pr = provider.createPullRequest("feature/a", "main", "PR", "Body")
    ...
    @Test
    fun `listOpenPullRequests filters by state`() =
        runTest {
            val provider = FakeVcsProvider()
