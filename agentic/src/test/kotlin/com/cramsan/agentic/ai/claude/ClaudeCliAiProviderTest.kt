...
class ClaudeCliAiProviderTest {
    ...
    @Test
    fun `successful response is wrapped in AiResponse with Text block`() =
        runTest {
            coEvery { shell.run(*anyVararg()) } returns
                ShellResult(
    ...
    @Test
    fun `non-zero exit code throws AiProviderException`() =
        runTest {
            coEvery { shell.run(*anyVararg()) } returns
                ShellResult(
                    stdout = "",
    ...
    @Test
    fun `non-empty tools list throws UnsupportedOperationException`() =
        runTest {
    ...
    @Test
    fun `non-zero exit code uses stdout when stderr is blank`() =
        runTest {
            coEvery { shell.run(*anyVararg()) } returns
                ShellResult(
    ...
    @Test
    fun `fullAccess=true invokes CLI with --dangerously-skip-permissions and --output-format json`() =
        runTest {
            coEvery { shell.run(*anyVararg()) } returns
                ShellResult(
    ...
    @Test
    fun `fullAccess=true parses result field from JSON output`() =
        runTest {
            coEvery { shell.run(*anyVararg()) } returns
                ShellResult(
    ...
    @Test
    fun `fullAccess=true is_error=true throws AiProviderException`() =
        runTest {
            coEvery { shell.run(*anyVararg()) } returns
                ShellResult(
    ...
    @Test
    fun `fullAccess=true does not throw on non-empty tools`() =
        runTest {
            coEvery { shell.run(*anyVararg()) } returns
                ShellResult(
