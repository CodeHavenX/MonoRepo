...
class DefaultValidationServiceExtendedTest {
    ...
    private fun makeDoc(id: String, type: DocumentType, file: String) =
        AgenticDocument(
            id = id,
            typeId = id,
            type = type,
    ...
    @Test
    fun `advisory-only issues result in VALIDATED not NEEDS_REVISION`() =
        runTest {
            ...
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
                    id = "r1",
                    stopReason = "end_turn",
                    content = listOf(AiContentBlock.Text(advisoryJson)),
                )
    ...
    @Test
    fun `mixed advisory and blocking issues results in NEEDS_REVISION`() =
        runTest {
            val doc = makeDoc("goals-scope", DocumentType.GOALS_SCOPE, "goals-scope.md")
            val mixedJson =
                """
            ...
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
                    id = "r1",
                    stopReason = "end_turn",
                    content = listOf(AiContentBlock.Text(mixedJson)),
                )
    ...
    @Test
    fun `empty JSON array response returns zero issues and marks VALIDATED`() =
        runTest {
    ...
    @Test
    fun `response with no Text content block returns zero issues`() =
        runTest {
            val doc = makeDoc("standards", DocumentType.STANDARDS, "standards.md")
    ...
    @Test
    fun `runValidationPass sets all docs to IN_REVIEW before reviewing any`() =
        runTest {
            val docs =
                listOf(
    ...
    @Test
    fun `reviewer agents do not change document status`() =
        runTest {
            ...
            coEvery {
                reviewerAgent.reviewDocuments(
                    reviewerDef,
                    docs,
                )
            } returns ReviewerFeedback("security", "Some findings")
    ...
    @Test
    fun `validation report file is created with run id and issue count`() =
        runTest {
            val docs = listOf(makeDoc("goals-scope", DocumentType.GOALS_SCOPE, "goals-scope.md"))
            every { documentStore.getAll() } returns docs
            ...
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
                    id = "r1",
                    stopReason = "end_turn",
                    content = listOf(AiContentBlock.Text(blockingIssue)),
                )
            every { reviewerLoader.loadAll() } returns emptyList()
    ...
    @Test
    fun `reviewDocument sends the document file content to the AI provider`() =
        runTest {
    ...
    @Test
    fun `reviewDocument uses empty tools list — no tool use in validation`() =
        runTest {
            val doc = makeDoc("standards", DocumentType.STANDARDS, "standards.md")
            coEvery { aiProvider.chat(any(), any(), any()) } returns
                AiResponse(
