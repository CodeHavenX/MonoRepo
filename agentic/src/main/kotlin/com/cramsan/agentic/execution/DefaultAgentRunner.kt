...
class DefaultAgentRunner(
    ...
    override suspend fun run(task: Task, worktree: Worktree): AgentResult =
        try {
            val result =
                withTimeout(task.timeoutSeconds.seconds) {
                    ...
                    reviewerAgents.map { agent ->
                        async {
                            try {
                                logI(
                                    TAG,
                                    "[REVIEW] Reviewer '${definition.name}' started review for task '${task.id}'.",
                                )
                                ...
                                logI(
                                    TAG,
                                    "[REVIEW] Reviewer '${definition.name}' completed review for task '${task.id}'.",
                                )
                            } catch (e: Exception) {
    ...
    private fun formatReviewerComment(feedback: com.cramsan.agentic.core.ReviewerFeedback): String = """<!-- agentic-reviewer: ${feedback.reviewerName} -->
