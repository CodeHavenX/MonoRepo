...
class RetryingAiProviderTest {
    ...
    @Test
    fun `delegates chat to underlying provider on success`() =
        runTest {
    ...
    @Test
    fun `rethrows non-limit AiProviderException immediately without retrying`() =
        runTest {
    ...
    @Test
    fun `rethrows non-AiProviderException immediately without retrying`() =
        runTest {
            val provider = RetryingAiProvider(delegate, maxRetries = 5, retryDelay = 0.seconds)
    ...
    @Test
    fun `retries on rate limit error and succeeds`() =
        runTest {
    ...
    @Test
    fun `retries on usage limit keyword and succeeds`() =
        runTest {
            val provider = RetryingAiProvider(delegate, maxRetries = 3, retryDelay = 0.seconds)
            var callCount = 0
            coEvery { delegate.chat(any(), any(), any()) } answers {
    ...
    @Test
    fun `retries on overloaded keyword`() =
        runTest {
            val provider = RetryingAiProvider(delegate, maxRetries = 3, retryDelay = 0.seconds)
            var callCount = 0
            coEvery { delegate.chat(any(), any(), any()) } answers {
    ...
    @Test
    fun `retries on too many requests keyword`() =
        runTest {
            val provider = RetryingAiProvider(delegate, maxRetries = 3, retryDelay = 0.seconds)
            var callCount = 0
            coEvery { delegate.chat(any(), any(), any()) } answers {
    ...
    @Test
    fun `throws after exhausting maxRetries on persistent usage limit`() =
        runTest {
            val provider = RetryingAiProvider(delegate, maxRetries = 3, retryDelay = 0.seconds)
    ...
    @Test
    fun `with maxRetries=0 does not retry at all`() =
        runTest {
