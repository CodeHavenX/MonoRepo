...
class LocalVcsProviderExtendedTest : CoroutineTest() {
    ...
    @Test
    fun `autoMerge=true causes createPullRequest to set state MERGED`() =
        runCoroutineTest {
    ...
    @Test
    fun `autoMerge=true createPullRequest runs git merge with correct arguments`() =
        runCoroutineTest {
            val provider = autoMergeProvider(testCoroutineDispatcher)
            coEvery { shell.run(*anyVararg(), workingDir = any()) } returns ShellResult("", 0, "")
    ...
    @Test
    fun `autoMerge=true merged PR appears in listMergedPullRequests`() =
        runCoroutineTest {
            val provider = autoMergeProvider(testCoroutineDispatcher)
            coEvery { shell.run(*anyVararg(), workingDir = any()) } returns ShellResult("", 0, "")
    ...
    @Test
    fun `autoMerge=true merged PR does not appear in listOpenPullRequests`() =
        runCoroutineTest {
            val provider = autoMergeProvider(testCoroutineDispatcher)
            coEvery { shell.run(*anyVararg(), workingDir = any()) } returns ShellResult("", 0, "")
    ...
    @Test
    fun `autoMerge=true isPullRequestMerged returns true without git check`() =
        runCoroutineTest {
            val provider = autoMergeProvider(testCoroutineDispatcher)
            coEvery { shell.run(*anyVararg(), workingDir = any()) } returns ShellResult("", 0, "")
    ...
    @Test
    fun `isPullRequestMerged with autoMerge=false runs git branch --merged`() =
        runCoroutineTest {
            val provider = manualMergeProvider(testCoroutineDispatcher)
            provider.createPullRequest("feature/a", "main", "PR 1", "Body")
            // Simulate: feature/a has been merged into main
            coEvery { shell.run(*anyVararg(), workingDir = any()) } returns
                ShellResult(
                    "  main\n  feature/a\n",
                    0,
                    "",
    ...
    @Test
    fun `isPullRequestMerged with autoMerge=false returns false when branch is not in merged list`() =
        runCoroutineTest {
            val provider = manualMergeProvider(testCoroutineDispatcher)
            provider.createPullRequest("feature/a", "main", "PR 1", "Body")
            coEvery { shell.run(*anyVararg(), workingDir = any()) } returns
    ...
    @Test
    fun `isPullRequestMerged with autoMerge=false updates state to MERGED when git confirms merge`() =
        runCoroutineTest {
            val provider = manualMergeProvider(testCoroutineDispatcher)
            provider.createPullRequest("feature/a", "main", "PR 1", "Body")
            coEvery { shell.run(*anyVararg(), workingDir = any()) } returns
            ...
            } throws AssertionError("git should not be called again")
            assertTrue(provider.isPullRequestMerged(pr.id))
        }

    @Test
    fun `isPullRequestMerged handles current branch marker in git branch output`() =
        runCoroutineTest {
            val provider = manualMergeProvider(testCoroutineDispatcher)
            provider.createPullRequest("feature/a", "main", "PR 1", "Body")
    ...
    @Test
    fun `state persists across separate LocalVcsProvider instances`() =
        runCoroutineTest {
    ...
    @Test
    fun `PR IDs continue incrementing across separate instances`() =
        runCoroutineTest {
            val providerA = manualMergeProvider(testCoroutineDispatcher)
    ...
    @Test
    fun `listOpenPullRequests returns PRs matching any of the given labels`() =
        runCoroutineTest {
            val provider = manualMergeProvider(testCoroutineDispatcher)
            provider.createPullRequest("feature/a", "main", "PR 1", "Body", listOf("agentic-code"))
    ...
    @Test
    fun `listMergedPullRequests with empty labels returns all merged PRs`() =
        runCoroutineTest {
            val provider = autoMergeProvider(testCoroutineDispatcher)
            coEvery { shell.run(*anyVararg(), workingDir = any()) } returns ShellResult("", 0, "")
    ...
    private fun autoMergeProvider(dispatcher: CoroutineDispatcher) =
        LocalVcsProvider(
            stateFile = stateFile,
            autoMerge = true,
            repoRoot = tempDir,
    ...
    private fun manualMergeProvider(dispatcher: CoroutineDispatcher) =
        LocalVcsProvider(
            stateFile = stateFile,
            autoMerge = false,
            repoRoot = tempDir,
