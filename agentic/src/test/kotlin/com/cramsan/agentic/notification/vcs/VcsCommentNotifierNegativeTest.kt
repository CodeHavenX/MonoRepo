...
class VcsCommentNotifierNegativeTest {
    ...
    private fun makeTask(id: String) =
        Task(
            id = id,
            title = "Task $id",
    ...
    private fun openPrForTask(taskId: String, prId: String = "pr-$taskId") =
        PullRequest(
            id = prId,
    ...
    @Test
    fun `TaskFailed propagates exception when listOpenPullRequests throws`() =
        runTest {
    ...
    @Test
    fun `TaskFailed propagates exception when getPullRequestComments throws`() =
        runTest {
    ...
    @Test
    fun `TaskFailed propagates exception when addPullRequestComment throws`() =
        runTest {
            coEvery { vcsProvider.listOpenPullRequests(any()) } returns listOf(openPrForTask("task-001"))
    ...
    @Test
    fun `TaskFailed does not post comment when open PR is for a different task`() =
        runTest {
    ...
    @Test
    fun `TaskFailed with empty reason still posts comment`() =
        runTest {
            coEvery { vcsProvider.listOpenPullRequests(any()) } returns listOf(openPrForTask("task-001"))
            coEvery { vcsProvider.getPullRequestComments(any()) } returns emptyList()
    ...
    @Test
    fun `RunDeadlocked does not post comment when no open PRs exist`() =
        runTest {
    ...
    @Test
    fun `RunDeadlocked propagates exception when listOpenPullRequests throws`() =
        runTest {
    ...
    @Test
    fun `RunDeadlocked with both blocked and failed lists empty still posts to oldest PR`() =
        runTest {
    ...
    @Test
    fun `RunDeadlocked posts to PR with lexicographically greatest id when multiple PRs exist`() =
        runTest {
    ...
    @Test
    fun `RunCompleted with empty task list makes no VCS calls`() =
        runTest {
    ...
    @Test
    fun `RunCompleted with multiple completed tasks makes no VCS calls`() =
        runTest {
